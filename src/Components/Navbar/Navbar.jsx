...
const Navbar = () => {
  ...
  const links = (
    <>
      <li>
        <NavLink className="hover:border-b-gray-500 hover:border-b-2" to="/">
          Home
        </NavLink>
      </li>
      <li>
        <NavLink
          className="hover:border-b-gray-500 hover:border-b-2"
          to="/allBooks"
        >
          All Books
        </NavLink>
  ...
  return (
    <div className="navbar shadow-sm dark:border-b-1 dark:border-gray-300 max-w-11/12 h-[90px] mx-auto mb-0">
      <div className="navbar-start">
        ...
        <div className=" flex items-center gap-1 font-semibold text-xl ">
          ...
          <p className="dark:text-white">
            Book<span className="text-gray-600 dark:text-gray-400">Haven</span>
          </p>
        </div>
      </div>
      <div className="navbar-center hidden lg:flex">
        <ul className="menu menu-horizontal px-1 font-semibold text-gray-700 dark:text-white ">
          {links}
        </ul>
      </div>
      <div className="navbar-end flex gap-1  items-center">
        ...
        >
          {theme === "light" ? (
            <MdDarkMode className="text-[30px] text-gray-700" />
          ) : (
            <MdLightMode className="text-[30px] text-gray-700" />
          )}
        </button>
        <div>
          {user && (
            ...
            >
              <img
                className="w-7.5  rounded-lg   border-gray-500"
        ...
        <div>
          {user ? (
            <button
              className="btn   rounded-4xl text-white bg-indigo-500"
